...
def nearest_neighbor_interpolation(
    tens: torch.tensor = torch.rand(2, 2), new_size: Tuple[int, ...] = (8, 8)
) -> torch.tensor:
    """
    ...
    assert (
        tens.shape <= new_size
    ), "Shape of new tensor '{0}' has to be equal or greator to original '{1}'".format(
        tens.shape, new_size
    )
    id_x = torch.arange(
        start=0, end=tens.shape[-1], step=(tens.shape[-1] / new_size[-1])
    ).to(dtype=torch.int64)
    id_y = torch.arange(
        start=0, end=tens.shape[-2], step=(tens.shape[-2] / new_size[-2])
    ).to(dtype=torch.int64)
...
def linear_interpolation(
    tens: torch.tensor = torch.ones(2), new_size: Tuple[int, ...] = (4,)
) -> torch.tensor:
    """
    ...
    assert tens.dim() == len(
        new_size
    ), "Tensor dim '{0}' has to be equal to new size dim '{1}'".format(
        tens.dim(), len(new_size)
    )
    immediate_interp_values = torch.linspace(
        start=0.0, end=1.0, steps=new_size[-1]
    ).unsqueeze(0)
